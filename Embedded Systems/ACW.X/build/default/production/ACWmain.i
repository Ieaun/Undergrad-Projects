...
_Bool FinishedSet;
_Bool Silencer=0; char letter='L';

char Heating_Letter=' ';

int Year_ds,Month_ds,Day_ds = 0;
_Bool ResetTime_date = 0;
...
void EnterData()
{
    ...
    if (ButtonPressed!=99)
    {
      ...
      switch (ActiveSet)
      {
              case 1: lcd_init();han_DSet();han_Display(0x90,display_SetTime);break;
              case 2: lcd_init();han_DSet();han_Display(0x90,display_SetDate);break;
              case 3: lcd_init();han_DSet();han_Display(0x90,display_SetTriggerTemp);break;
      }
      while(FinishedSet == 1)
       {
       keymatrix(&ButtonPressed,&ActiveSet);
       if(ButtonPressed<98)
            {
            lcd_init();han_Number(0x93,display_number[ButtonPressed]);
            GetValue(display_number[ButtonPressed]);
...
void BiggerThanX(int NumberChars)
{
    ...
    if (CurrentPosValue>NumberChars)
    {
         switch (ActiveSet)
      {
              case 1: if (AllValues[0][0]!=0x3f){if (AllValues[0][0]>'2' || (AllValues[0][0]>='2' && AllValues[0][1]>'3') || AllValues[0][2]>='6' ){lcd_init();CallInvalid();ArrayReset(0);break;}}
